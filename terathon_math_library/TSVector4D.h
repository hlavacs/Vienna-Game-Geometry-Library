...
namespace Terathon
{
	...
	class Vector4D : public Vec4D<TypeVector4D>
	{
		public:
			...
			Vector4D(real a, real b, real c, real d) : Vec4D<TypeVector4D>(a, b, c, d) {}

			template <typename type>
			explicit Vector4D(const Vec4D<type>& v) : Vec4D<TypeVector4D>(real(v.x), real(v.y), real(v.z), real(v.w)) {}

			Vector4D(const Vector3D& v, real d)
			{
				xyz = v.xyz;
				w = d;
			}
			...
			Vector4D& Set(real a, real b, real c, real d)
			{
				xyzw.Set(a, b, c, d);
				return (*this);
			}
			...
			Vector4D& Set(const Vector3D& v, real d)
			{
				xyzw.Set(v.x, v.y, v.z, d);
				return (*this);
			}

			void Set(const Vector3D& v, real d) volatile
			{
				xyzw.Set(v.x, v.y, v.z, d);
			}
			...
			Vector4D& Set(const Vector2D& v, real c, real d)
			{
				xyzw.Set(v.x, v.y, c, d);
				return (*this);
			}

			void Set(const Vector2D& v, real c, real d) volatile
			{
				xyzw.Set(v.x, v.y, c, d);
			}
			...
			TERATHON_API Vector4D& RotateAboutAxis(real angle, const Bivector3D& axis);
	};
	...
	inline Vector4D operator *(const Vector4D& v, real n)
	{
	...
	inline Vector4D operator *(real n, const Vector4D& v)
	{
	...
	inline real Magnitude(const Vector4D& v)
	{
	...
	inline real InverseMag(const Vector4D& v)
	{
	...
	inline real SquaredMag(const Vector4D& v)
	{
	...
	inline real Dot(const Vector4D& a, const Vector3D& b)
	{
		return (a.x * b.x + a.y * b.y + a.z * b.z);
	}

	inline real Dot(const Vector3D& a, const Vector4D& b)
	{
		return (a.x * b.x + a.y * b.y + a.z * b.z);
	}

	inline real Dot(const Vector4D& v, const Point3D& p)
	{
		return (v.x * p.x + v.y * p.y + v.z * p.z + v.w);
	}

	inline real Dot(const Point3D& a, const Vector4D& b)
	{
		return (a.x * b.x + a.y * b.y + a.z * b.z + b.w);
	}

	inline real Dot(const Vector4D& a, const Vector2D& b)
	{
		return (a.x * b.x + a.y * b.y);
	}

	inline real Dot(const Vector2D& a, const Vector4D& b)
	{
		return (a.x * b.x + a.y * b.y);
	}

	inline real Dot(const Vector4D& v, const Point2D& p)
	{
		return (v.x * p.x + v.y * p.y + v.w);
	}

	inline real Dot(const Point2D& a, const Vector4D& b)
	{
