...
namespace Terathon
{
	...
	class Flector3D
	{
		public:
			...
			Flector3D(real px, real py, real pz, real pw, real gx, real gy, real gz, real gw)
			{
				p.Set(px, py, pz, pw);
				g.Set(gx, gy, gz, gw);
			}
			...
			Flector3D& Set(real px, real py, real pz, real pw, real gx, real gy, real gz, real gw)
			{
				p.Set(px, py, pz, pw);
				g.Set(gx, gy, gz, gw);
				return (*this);
			...
			void Set(real px, real py, real pz, real pw, real gx, real gy, real gz, real gw) volatile
			{
				p.Set(px, py, pz, pw);
				g.Set(gx, gy, gz, gw);
			}
			...
			static Flector3D MakeRotoreflection(real angle, const Line3D& axis, const Plane3D& plane)
			{
				Vector2D t = CosSin(angle * 0.5F);
				real vx = axis.v.x * t.y;
				real vy = axis.v.y * t.y;
				real vz = axis.v.z * t.y;
				real mx = axis.m.x * t.y;
				real my = axis.m.y * t.y;
				real mz = axis.m.z * t.y;
			...
			static Flector3D MakeRotoreflection(real angle, const Bivector3D& axis, const Plane3D& plane)
			{
				Vector2D t = CosSin(angle * 0.5F);
				real vx = axis.x * t.y;
				real vy = axis.y * t.y;
	...
	inline Flector3D operator *(const Flector3D& F, real n)
	{
	...
	inline Flector3D operator *(real n, const Flector3D& F)
	{
	...
	inline real SquaredBulkNorm(const Flector3D& F)
	{
	...
	inline real SquaredWeightNorm(const Flector3D& F)
	{
