...
Matrix3D::Matrix3D(real n00, real n01, real n02, real n10, real n11, real n12, real n20, real n21, real n22) : Mat3D<TypeMatrix3D>(n00, n01, n02, n10, n11, n12, n20, n21, n22)
{
}
...
Matrix3D& Matrix3D::Set(real n00, real n01, real n02, real n10, real n11, real n12, real n20, real n21, real n22)
{
...
{
	real t = m00 * m(0,0) + m01 * m(1,0) + m02 * m(2,0);
	real u = m00 * m(0,1) + m01 * m(1,1) + m02 * m(2,1);
...
void Matrix3D::GetEulerAngles(real *x, real *y, real *z) const
{
	real sy = m02;
	if (sy < 1.0F)
	{
...
Matrix3D& Matrix3D::SetEulerAngles(real x, real y, real z)
{
...
Matrix3D Matrix3D::MakeRotationX(real angle)
{
...
Matrix3D Matrix3D::MakeRotationY(real angle)
{
	Vector2D v = CosSin(angle);
...
Matrix3D Matrix3D::MakeRotationZ(real angle)
{
	Vector2D v = CosSin(angle);
...
Matrix3D Matrix3D::MakeRotation(real angle, const Bivector3D& axis)
{
	real	c, s;
...
Matrix3D Matrix3D::MakeScale(real scale)
{
	return (Matrix3D(scale, 0.0F, 0.0F, 0.0F, scale, 0.0F, 0.0F, 0.0F, scale));
}

Matrix3D Matrix3D::MakeScale(real sx, real sy, real sz)
{
	return (Matrix3D(sx, 0.0F, 0.0F, 0.0F, sy, 0.0F, 0.0F, 0.0F, sz));
}

Matrix3D Matrix3D::MakeScale(real scale, const Vector3D& a)
{
	scale -= 1.0F;
	real x = a.x * scale;
	real y = a.y * scale;
...
Matrix3D Matrix3D::MakeScaleX(real sx)
{
	return (Matrix3D(sx, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 1.0F));
}

Matrix3D Matrix3D::MakeScaleY(real sy)
{
	return (Matrix3D(1.0F, 0.0F, 0.0F, 0.0F, sy, 0.0F, 0.0F, 0.0F, 1.0F));
}

Matrix3D Matrix3D::MakeScaleZ(real sz)
{
	return (Matrix3D(1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, sz));
}

Matrix3D Matrix3D::MakeSkew(real angle, const Vector3D& a, const Vector3D& b)
{
	real t = Tan(angle);
	real x = a.x * t;
	real y = a.y * t;
...
Matrix3D Terathon::operator *(const Matrix3D& m, real s)
{
...
real Terathon::Determinant(const Matrix3D& m)
{
...
Transform2D::Transform2D(real n00, real n01, real n02, real n10, real n11, real n12)
{
	m00 = n00;
	m01 = n01;
	m02 = n02;
...
Transform2D::Transform2D(const Vector2D& r0, real n02, const Vector2D& r1, real n12)
{
...
Transform2D& Transform2D::Set(real n00, real n01, real n02, real n10, real n11, real n12)
{
	m00 = n00;
	m01 = n01;
	m02 = n02;
...
Transform2D Transform2D::MakeRotation(real angle)
{
	Vector2D v = CosSin(angle);
...
Transform2D Transform2D::MakeScale(real sx, real sy)
{
...
real Terathon::Determinant(const Transform2D& m)
{
...
{
	real invDet = 1.0F / (m(0,0) * m(1,1) - m(0,1) * m(1,0));
