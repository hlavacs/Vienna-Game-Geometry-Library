...
namespace Terathon
{
	...
	class Vector2D : public Vec2D<TypeVector2D>
	{
		public:
			...
			Vector2D(real a, real b) : Vec2D<TypeVector2D>(a, b) {}

			template <typename type>
			explicit Vector2D(const Vec2D<type>& v) : Vec2D<TypeVector2D>(real(v.x), real(v.y)) {}

			/// \brief Sets both components of a 2D vector.
			/// \param a,b		The new components of the vector.

			Vector2D& Set(real a, real b)
			{
				xy.Set(a, b);
				return (*this);
			}

			void Set(real a, real b) volatile
			{
				xy.Set(a, b);
			}
			...
			TERATHON_API Vector2D& Rotate(real angle);
	};
	...
	inline Vector2D operator *(const Vector2D& v, real n)
	{
	...
	inline Vector2D operator *(real n, const Vector2D& v)
	{
	...
	class Point2D : public Vector2D
	{
		public:
			...
			Point2D(real a, real b) : Vector2D(a, b) {}
	...
	inline Point2D operator *(const Point2D& p, real n)
	{
	...
	inline Point2D operator *(real n, const Point2D& p)
	{
	...
	inline real Magnitude(const Vector2D& v)
	{
	...
	inline real InverseMag(const Vector2D& v)
	{
	...
	inline real SquaredMag(const Vector2D& v)
	{
	...
	inline real Dot(const Point2D& a, const Point2D& b)
	{
		return (a.x * b.x + a.y * b.y);
	}

	inline real Dot(const Point2D& p, const Vector2D& v)
	{
		return (p.x * v.x + p.y * v.y);
	}

	inline real Dot(const Vector2D& v, const Point2D& p)
	{
	...
	inline real Antiwedge(const Vector2D& a, const Vector2D& b)
	{
		return (a.x * b.y - a.y * b.x);
	}

	inline real operator ^(const Vector2D& a, const Vector2D& b) {return (Antiwedge(a, b));}
