...
namespace Terathon
{
	...
	class Matrix2D : public Mat2D<TypeMatrix2D>
	{
		public:
			...
			TERATHON_API Matrix2D(real n00, real n01, real n10, real n11);
			...
			TERATHON_API Matrix2D& Set(real n00, real n01, real n10, real n11);
			...
			TERATHON_API Matrix2D& operator *=(real s);
			TERATHON_API Matrix2D& operator /=(real s);
			...
			TERATHON_API static Matrix2D MakeRotation(real angle);

			/// \brief Returns a 2&#x202F;&times;&#x202F;2 matrix that represents a uniform scale.
			/// \param scale	The scale along both axes.

			TERATHON_API static Matrix2D MakeScale(real scale);

			TERATHON_API static Matrix2D MakeScale(real sx, real sy);
			TERATHON_API static Matrix2D MakeScaleX(real sx);
			TERATHON_API static Matrix2D MakeScaleY(real sy);
	};
	...
	TERATHON_API Matrix2D operator *(const Matrix2D& m, real s);
	TERATHON_API Matrix2D operator /(const Matrix2D& m, real s);

	inline Matrix2D operator *(real s, const Matrix2D& m)
	{
		return (m * s);
	}
	...
	TERATHON_API real Determinant(const Matrix2D& m);
