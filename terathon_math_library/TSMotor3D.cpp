...
{
	real A00 = 1.0F - (v.y * v.y + v.z * v.z) * 2.0F;
	real A01 = v.x * v.y;
	real A02 = v.z * v.x;
	real B01 = v.z * v.w;
	real B20 = v.y * v.w;
...
{
	real A11 = 1.0F - (v.z * v.z + v.x * v.x) * 2.0F;
	real A01 = v.x * v.y;
	real A12 = v.y * v.z;
	real B01 = v.z * v.w;
	real B12 = v.x * v.w;
...
{
	real A22 = 1.0F - (v.x * v.x + v.y * v.y) * 2.0F;
	real A02 = v.z * v.x;
	real A12 = v.y * v.z;
	real B20 = v.y * v.w;
	real B12 = v.x * v.w;
...
{
	real A03 = v.y * m.z - v.z * m.y;
	real A13 = v.z * m.x - v.x * m.z;
	real A23 = v.x * m.y - v.y * m.x;
	real B03 = m.x * v.w - v.x * m.w;
	real B13 = m.y * v.w - v.y * m.w;
	real B23 = m.z * v.w - v.z * m.w;
...
Transform3D Motor3D::GetTransformMatrix(void) const
{
	...
	real A00 = 1.0F - (vy2 + vz2) * 2.0F;
	real A11 = 1.0F - (vz2 + vx2) * 2.0F;
	real A22 = 1.0F - (vx2 + vy2) * 2.0F;
	real A01 = v.x * v.y;
	real A02 = v.z * v.x;
	real A12 = v.y * v.z;
	real A03 = v.y * m.z - v.z * m.y;
...
{
	real b = InverseSqrt(Q.v.w * 2.0F + 2.0F);
	real a = -Q.m.w * (b * b);
...
Plane3D Terathon::Transform(const Plane3D& g, const Motor3D& Q)
{
	...
	real bv = b.x * Q.v.x + b.y * Q.v.y + b.z * Q.v.z;
	real mg = Q.m.x * g.x + Q.m.y * g.y + Q.m.z * g.z;
