...
namespace Terathon
{
	...
	class FlatPoint2D : public Vector3D
	{
		public:
			...
			FlatPoint2D(real a, real b, real c) : Vector3D(a, b, c) {}

			FlatPoint2D(const Point2D& p) : Vector3D(p) {}
			FlatPoint2D(const Vector2D& v) : Vector3D(v) {}
			FlatPoint2D(const Vector2D& v, real c) : Vector3D(v, c) {}
	...
	inline FlatPoint2D operator *(const FlatPoint2D& p, real n)
	{
	...
	inline FlatPoint2D operator *(real n, const FlatPoint2D& p)
	{
	...
	class Line2D : public Antivec3D<TypeLine2D>
	{
		public:
			...
			Line2D(real a, real b, real c) : Antivec3D<TypeLine2D>(a, b, c) {}
			...
			Line2D& Set(real a, real b, real c)
			{
				xyz.Set(a, b, c);
				return (*this);
			}
	...
	inline Line2D operator *(const Line2D& g, real n)
	{
	...
	inline Line2D operator *(real n, const Line2D& g)
	{
	...
	inline real Attitude(const FlatPoint2D& p)
	{
		return (p.z);
	}
	...
	inline real SquaredBulkNorm(const FlatPoint2D& p)
	{
	...
	inline real SquaredBulkNorm(const Line2D& g)
	{
		return (g.z * g.z);
	}
	...
	inline real SquaredWeightNorm(const FlatPoint2D& p)
	{
		return (p.z * p.z);
	}
	...
	inline real SquaredWeightNorm(const Line2D& g)
	{
	...
	inline real Dot(const FlatPoint2D& a, const FlatPoint2D& b)
	{
	...
	inline real Dot(const Line2D& g, const Line2D& h)
	{
		return (g.z * h.z);
	}
	...
	inline real Antidot(const FlatPoint2D& a, const FlatPoint2D& b)
	{
		return (a.z * b.z);
	}
	...
	inline real Antidot(const Line2D& g, const Line2D& h)
	{
	...
	inline real Antiwedge(const Point2D& p, const Line2D& g)
	{
		return (-p.x * g.x - p.y * g.y - g.z);
	}

	inline real Antiwedge(const Line2D& g, const Point2D& p)
	{
		return (-p.x * g.x - p.y * g.y - g.z);
	}

	inline FlatPoint2D operator ^(const Line2D& g, const Line2D& h) {return (Antiwedge(g, h));}
	inline real operator ^(const Point2D& p, const Line2D& g) {return (Antiwedge(p, g));}
	inline real operator ^(const Line2D& g, const Point2D& p) {return (Antiwedge(g, p));}
