...
Transform3D Flector3D::GetTransformMatrix(void) const
{
	...
	real A00 = (gy2 + gz2) * 2.0F - 1.0F;
	real A11 = (gz2 + gx2) * 2.0F - 1.0F;
	real A22 = (gx2 + gy2) * 2.0F - 1.0F;
	real A01 = g.x * g.y * -2.0F;
	real A02 = g.z * g.x * -2.0F;
	real A12 = g.y * g.z * -2.0F;
	real A03 = p.x * p.w - g.x * g.w;
	real A13 = p.y * p.w - g.y * g.w;
	real A23 = p.z * p.w - g.z * g.w;

	real B01 = g.z * p.w * 2.0F;
	real B20 = g.y * p.w * 2.0F;
	real B12 = g.x * p.w * 2.0F;
	real B03 = g.y * p.z - g.z * p.y;
	real B13 = g.z * p.x - g.x * p.z;
	real B23 = g.x * p.y - g.y * p.x;
...
{
	real hp = h.x * F.p.x + h.y * F.p.y + h.z * F.p.z;
	real hg = h.x * F.g.x + h.y * F.g.y + h.z * F.g.z;
