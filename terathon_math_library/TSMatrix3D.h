...
namespace Terathon
{
	...
	class Matrix3D : public Mat3D<TypeMatrix3D>
	{
		public:
			...
			TERATHON_API Matrix3D(real n00, real n01, real n02, real n10, real n11, real n12, real n20, real n21, real n22);
			...
			TERATHON_API Matrix3D& Set(real n00, real n01, real n02, real n10, real n11, real n12, real n20, real n21, real n22);
			...
			TERATHON_API Matrix3D& operator *=(real s);
			TERATHON_API Matrix3D& operator /=(real s);
			...
			TERATHON_API Matrix3D& SetEulerAngles(real x, real y, real z);
			...
			TERATHON_API static Matrix3D MakeRotation(real angle, const Bivector3D& axis);
			...
			TERATHON_API static Matrix3D MakeScale(real scale);
			TERATHON_API static Matrix3D MakeScale(real sx, real sy, real sz);
			TERATHON_API static Matrix3D MakeScale(real scale, const Vector3D& a);
			TERATHON_API static Matrix3D MakeScaleX(real sx);
			TERATHON_API static Matrix3D MakeScaleY(real sy);
			TERATHON_API static Matrix3D MakeScaleZ(real sz);

			TERATHON_API static Matrix3D MakeSkew(real angle, const Vector3D& a, const Vector3D& b);
	};
	...
	TERATHON_API Matrix3D operator *(const Matrix3D& m, real s);
	TERATHON_API Matrix3D operator /(const Matrix3D& m, real s);

	inline Matrix3D operator *(real s, const Matrix3D& m)
	{
		return (m * s);
	}
	...
	TERATHON_API real Determinant(const Matrix3D& m);
	...
	class Transform2D : public Matrix3D
	{
		public:
			...
			TERATHON_API Transform2D(real n00, real n01, real n02, real n10, real n11, real n12);

			TERATHON_API Transform2D(const Vector2D& a, const Vector2D& b, const Point2D& p);
			TERATHON_API Transform2D(const Vector2D& r0, real n02, const Vector2D& r1, real n12);
			TERATHON_API Transform2D(const Matrix2D& m);
			TERATHON_API Transform2D(const Matrix2D& m, const Vector2D& v);

			TERATHON_API Transform2D& Set(real n00, real n01, real n02, real n10, real n11, real n12);
			...
			Transform2D& SetTranslation(real x, real y)
			{
				m02 = x;
				m12 = y;
				return (*this);
			...
			TERATHON_API static Transform2D MakeRotation(real angle);
			TERATHON_API static Transform2D MakeScale(real sx, real sy);
	...
	TERATHON_API real Determinant(const Transform2D& m);
