...
namespace Terathon
{
	...
	class Vector3D : public Vec3D<TypeVector3D>
	{
		public:
			...
			Vector3D(real a, real b, real c) : Vec3D<TypeVector3D>(a, b, c) {}

			template <typename type>
			explicit Vector3D(const Vec3D<type>& v) : Vec3D<TypeVector3D>(real(v.x), real(v.y), real(v.z)) {}
			...
			Vector3D(const Vector2D& v, real c)
			{
				xy = v.xy;
				z = c;
			}
			...
			Vector3D& Set(real a, real b, real c)
			{
				xyz.Set(a, b, c);
				return (*this);
			}

			void Set(real a, real b, real c) volatile
			{
				xyz.Set(a, b, c);
			}

			Vector3D& Set(const Vector2D& v, real c)
			{
				xyz.Set(v.x, v.y, c);
				return (*this);
			}

			void Set(const Vector2D& v, real c) volatile
			{
				xyz.Set(v.x, v.y, c);
			}
			...
			TERATHON_API Vector3D& RotateAboutX(real angle);

			/// \brief Rotates the vector about the <i>y</i> axis through the
			/// angle, in radians, given by the $angle$ parameter.

			TERATHON_API Vector3D& RotateAboutY(real angle);

			/// \brief Rotates the vector about the <i>z</i> axis through the
			/// angle, in radians, given by the $angle$ parameter.

			TERATHON_API Vector3D& RotateAboutZ(real angle);
			...
			TERATHON_API Vector3D& RotateAboutAxis(real angle, const Bivector3D& axis);
	};
	...
	inline Vector3D operator *(const Vector3D& v, real n)
	{
	...
	inline Vector3D operator *(real n, const Vector3D& v)
	{
	...
	template <typename type_struct, int count, int index_x, int index_y, int index_z>
	inline real operator ^(const Subvec3D<type_struct, true, count, index_x, index_y, index_z>& a, const Vector3D& b)
	{
	...
	inline real operator ^(const Subvec4D<type_struct, true, count, index_x, index_y, index_z, index_w>& a, const Vector3D& b)
	{
		return (a.data[index_x] * b.x + a.data[index_y] * b.y + a.data[index_z] * b.z);
	}
	...
	class Point3D : public Vector3D
	{
		public:
			...
			Point3D(real a, real b, real c) : Vector3D(a, b, c) {}

			Point3D(const Vector2D& v) : Vector3D(v) {}
			Point3D(const Vector2D& v, real c) : Vector3D(v, c) {}
	...
	inline Point3D operator *(const Point3D& p, real n)
	{
	...
	inline Point3D operator *(real n, const Point3D& p)
	{
	...
	inline real Magnitude(const Vector3D& v)
	{
	...
	inline real InverseMag(const Vector3D& v)
	{
	...
	inline real SquaredMag(const Vector3D& v)
	{
	...
	inline real Dot(const Point3D& a, const Point3D& b)
	{
		return (a.x * b.x + a.y * b.y + a.z * b.z);
	}

	inline real Dot(const Point3D& p, const Vector3D& v)
	{
		return (p.x * v.x + p.y * v.y + p.z * v.z);
	}

	inline real Dot(const Vector3D& v, const Point3D& p)
	{
		return (v.x * p.x + v.y * p.y + v.z * p.z);
	}

	template <typename type_struct, int count, int index_x, int index_y, int index_z, int index_w>
	inline real operator ^(const Subvec4D<type_struct, true, count, index_x, index_y, index_z, index_w>& a, const Point3D& b)
	{
