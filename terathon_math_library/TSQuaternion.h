...
namespace Terathon
{
	...
	class Quaternion
	{
		public:
			...
			Quaternion(const Bivector3D& v, real s)
			{
				xyz = v;
				w = s;
			}
			...
			void Set(real a, real b, real c, real s) volatile
			{
				xyz.Set(a, b, c);
				w = s;
			}

			Quaternion& Set(const Bivector3D& v, real s)
			{
				xyz = v;
				w = s;
				return (*this);
			...
			void Set(const Bivector3D& v, real s) volatile
			{
				xyz = v;
				w = s;
			}
			...
			static Quaternion MakeRotationX(real angle)
			{
			...
			static Quaternion MakeRotationY(real angle)
			{
				Vector2D v = CosSin(angle * 0.5F);
			...
			static Quaternion MakeRotationZ(real angle)
			{
				Vector2D v = CosSin(angle * 0.5F);
			...
			static Quaternion MakeRotation(real angle, const Bivector3D& axis)
			{
				Vector2D v = CosSin(angle * 0.5F);
	...
	inline bool operator ==(const Quaternion& q, real s)
	{
		return ((q.w == s) && (q.x == 0.0F) && (q.y == 0.0F) && (q.z == 0.0F));
	}

	inline bool operator ==(real s, const Quaternion& q)
	{
		return ((q.w == s) && (q.x == 0.0F) && (q.y == 0.0F) && (q.z == 0.0F));
	}
	...
	inline real Magnitude(const Quaternion& q)
	{
	...
	inline real InverseMag(const Quaternion& q)
	{
	...
	inline real SquaredMag(const Quaternion& q)
	{
